...
class HrPunishment(models.Model):
    ...
    p_type = fields.Selection([
        ('warning', 'Warning'),
        ('penalty', 'Penalty'),
        ('termination', 'Termination')], default='warning',
        required=True, string="Type", tracking=True)
    punishment_type = fields.Selection([
        ('fixed_amount', 'Fixed Amount'),
        ('depend_on_salary', 'Depends on Salary')], default='fixed_amount', string="Punishment Type",
        tracking=True)
    penalty_amt = fields.Float(string='Fixed Amount', default=0.0, tracking=True)
    punishment_type_amount = fields.Selection([
        ('duration', 'Duration'),
        ('percentage', 'Percentage')], default='duration',
        string="Amount of Penalty Type", tracking=True)
    penalty_duration = fields.Float(string='Duration (In Days)', default=0.0, tracking=True)
    penalty_percentage = fields.Float(string='Percentage', default=0.0, tracking=True)
    # pen_allowance = fields.Many2one('hr.salary.rule', string='Allowances', tracking=True)
