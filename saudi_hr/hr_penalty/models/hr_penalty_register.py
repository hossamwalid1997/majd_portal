...
class HrPenaltyRegister(models.Model):
    ...
    penalty_id = fields.Many2one('hr.penalty', string='Penalty', required=True, tracking=True)
    punishment_ids = fields.Many2many('hr.punishment', required=True, string="Punishment")
    penalty_number = fields.Selection([
        ('first_time', 'First Time'),
        ('second_time', 'Second Time'),
        ('third_time', 'Third Time'),
        ('fourth_time', 'Fourth Time'),
        ('fifth_time', 'Fifth Time')], string="Penalty Number", tracking=True)
    deduction_value = fields.Selection([
        ('ded_salary', 'Deduction from Salary'),
        ('ded_deduction', 'Direct Payment Deduction')],
        default='ded_salary', string="Deduction Value", tracking=True)
    deduction_amt = fields.Float(string='Deduction Amount',
       required=True, default=0.0, copy=False, tracking=True)
    # pen_deduction = fields.Many2one('hr.salary.rule', string='Deduction', tracking=True)
    decision = fields.Text('Decision', tracking=True)
    acc_committee_ids = fields.Many2many('hr.employee', 'emp_pen_rel', 'emp_id', 'penalty_id',
    ...
    def action_done(self):
        for rec in self:
            punishment_termination = rec.punishment_ids.filtered(lambda p: p.p_type == 'termination')
    ...
    def action_create_warning(self):
        ...
        for punishment in self.punishment_ids.filtered(lambda p: p.p_type == 'warning'):
            ...
            warning_data.update({'target_group': 'employee',
                                 'employee_id': self.employee_id.id,
                                 'warning_action': 'expiry',
                                 'pen_reg_id': self.id,
                                 'description': punishment.name,
            })
            warning_id = self.env['issue.warning'].create(warning_data)
    ...
    def action_create_payment(self):
        for rec in self:
            ...
            if not rec.payment_id:
                rec.payment_id = self.env['account.payment'].create(account_payment_data)
    ...
    def _onchange_penalty(self):
        for rec in self:
            if rec.penalty_id and rec.penalty_id.first_time and rec.penalty_number == 'first_time':
                rec.punishment_ids = rec.penalty_id.first_time

            if rec.penalty_id and rec.penalty_id.second_time and rec.penalty_number == 'second_time':
                rec.punishment_ids = rec.penalty_id.second_time

            if rec.penalty_id and rec.penalty_id.third_time and rec.penalty_number == 'third_time':
                rec.punishment_ids = rec.penalty_id.third_time

            if rec.penalty_id and rec.penalty_id.fourth_time and rec.penalty_number == 'fourth_time':
                rec.punishment_ids = rec.penalty_id.fourth_time

            if rec.penalty_id and rec.penalty_id.fifth_time and rec.penalty_number == 'fifth_time':
    ...
    def _onchange_punishment_ids(self):
        for rec in self:
            ...
            for punishment in rec.punishment_ids.filtered(lambda p: p.p_type == 'penalty'):
                if punishment.punishment_type == 'fixed_amount':
                    deduction_amt += punishment.penalty_amt
                else:
                    if punishment.punishment_type_amount == 'duration' and wage and punishment.penalty_duration:
                        deduction_amt += (wage/30 * punishment.penalty_duration)
                    elif punishment.punishment_type_amount == 'percentage' and wage and punishment.penalty_percentage:
    ...
    def view_payslip(self):
        ...
        """
        self.ensure_one()
        tree_view = self.env.ref('hr_payroll.view_hr_payslip_tree')
