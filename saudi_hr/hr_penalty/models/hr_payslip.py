...
class HrPayslip(models.Model):
    ...
    pen_reg_ids = fields.Many2many('hr.penalty.register', 'payslip_punishment_rel',
        'payslip_id', 'punishment_id', string="Penalty")

    def compute_sheet(self):
        res = super(HrPayslip, self).compute_sheet()
        for payslip in self:
            slip_line_ids = self.env['hr.payslip.line'].search([('slip_id', '=', payslip.id),
                ('code', '=', 'PEN')])
            if slip_line_ids:
                slip_line_ids.unlink()

            pen_ids = self.env['hr.penalty.register'].search([('employee_id', '=', payslip.employee_id.id),
                ('state', '=', 'done'),
                ('is_ded', '=', True),
            ...
            if rule_ids and d_amt:
                rule = rule_ids[0]
                slip_line_data = {'slip_id': payslip.id,
                ...
                net_ids = self.env['hr.payslip.line'].search([('slip_id', '=', payslip.id), ('code', '=', 'NET')])
                if net_ids:
