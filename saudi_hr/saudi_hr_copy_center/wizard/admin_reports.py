...
class AdminReports(models.TransientModel):
    ...
    report = fields.Selection(selection_add=[('copy_center', 'Copy Center')], ondelete={'copy_center': 'cascade'})

    def print_copy_center_report(self):
        report = self
        month = report.date_from.strftime('%m/%y') if report.date_from else False
        copy_center_pool = self.env['copy.center']

        filename = '%s.xlsx' % ('Copy Center of month %s' % month) if month else 'Copy Center.xlsx'
        fp = BytesIO()
        ...
        for copy in copy_center_pool.search(domain):
            ...
            line_dict = {'Request Type': copy.request_type or '',
                         ...
                         'Submit Date': datetime.strftime(copy.date_submited, DEFAULT_SERVER_DATE_FORMAT) or '',
