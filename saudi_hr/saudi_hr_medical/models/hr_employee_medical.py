...
class InsuranceDetails(models.Model):
    ...
    premium_type = fields.Selection([('monthly', 'Monthly'),
                                     ('quarterly', 'Quarterly'),
                                     ('half', 'Half Yearly'),
                                     ('yearly', 'Yearly')], string='Payment Mode', required=True, default='monthly', tracking=True)
...
class InsurancePremium(models.Model):
    ...
    branch_id = fields.Many2one('hr.branch', string='Office', related='employee_id.branch_id', store=True)
    department_id = fields.Many2one('hr.department', string='Department', related='employee_id.department_id', store=True)
    ...
    def create_invoice(self):
        ...
        invoices_id._compute_tax_totals_json()
...
class ClaimDetails(models.Model):
    ...
    date_applied = fields.Date('Date Applied', default=fields.Date.today(), required=True, tracking=True)
    claim_amount = fields.Float('Claim Amount', required=True, tracking=True)
    passed_amount = fields.Float('Passed Amount', tracking=True)
    insurance_id = fields.Many2one('insurance.details', string='Insurance', required=True, domain="[('state', '=', 'confirmed'), ('employee_id', '=', employee_id)]", tracking=True)
    company_id = fields.Many2one('res.company', string="Company", compute=_set_insurance_vals, store=True, tracking=True)
    responsible_id = fields.Many2one('res.users', string="Responsible", required=True, default=lambda self: self.env.uid, tracking=True)
