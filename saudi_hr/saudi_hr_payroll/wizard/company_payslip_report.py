        ...
        if context.get('active_model', False) and context.get('active_id', False):
            ...
            for payslip in payslip_batch.slip_ids:
                ...
                for slip_rule in payslip.line_ids:
                    if slip_rule.code in rule_codes or (slip_rule.code[0:4] == 'LOAN' and slip_rule.code[0:4] in rule_codes):
                        if slip_rule.code[0:4] == 'LOAN':
                            worksheet.write(row, rule_column_dict[slip_rule.code[0:4]], slip_rule.total)
                        else:
