...
class HrEmployeeEos(models.Model):
    ...
    date_valid = fields.Date('Validation Date', index=True, copy=False, help="Date of the acceptation of the sheet eos. It's filled when the button Validate is pressed.", readonly=True)
    date_approve = fields.Date('Approve Date', index=True, copy=False, help="Date of the Approval of the sheet eos. It's filled when the button Approve is pressed.", readonly=True)
    user_valid = fields.Many2one('res.users', 'Validation by', copy=False, readonly=True, states={'draft': [('readonly', False)], 'confirm': [('readonly', False)]}, store=True)
    user_approve = fields.Many2one('res.users', 'Approval by', copy=False, readonly=True, states={'draft': [('readonly', False)], 'confirm': [('readonly', False)]}, store=True)
    ...
    def calc_eos(self):
        ...
        for eos in self:
            ...
            eos.payslip_id._compute_worked_days_line_ids()
    ...
    def action_receipt_create(self):
        ...
        for eos in self:
            ...
            debit_account_id = eos.journal_id.company_id.account_journal_payment_credit_account_id.id
            credit_account_id = eos.employee_id.address_home_id.property_account_payable_id.id
    ...
    def action_view_receipt(self):
        ...
        try:
            dummy, view_id = self.env['ir.model.data']._xmlid_lookup('account.view_move_form')
        except ValueError:
            view_id = False
        result = {
