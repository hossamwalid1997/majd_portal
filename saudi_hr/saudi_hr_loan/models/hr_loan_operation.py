...
class HrLoanOperation(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', 'Employee', required=True, tracking=True, domain=lambda self: self._get_employee_domain())
    department_id = fields.Many2one('hr.department', string="Department", related='employee_id.department_id', store=True)
    branch_id = fields.Many2one('hr.branch', 'Office', readonly=True, related='employee_id.branch_id', store=True)
    ...
    loan_next_schedule_id = fields.Many2one('hr.employee.loan.line', string="Loan Schedule", copy=False)
    ...
    def check_loan_operation_req(self, employee, effective_date, type):
        if effective_date:
            loan_operation_ids = self.search([('state', 'in', ['confirm', 'open', 'approve']), ('employee_id', '=', employee.id), ('id', '!=', self.id), ('loan_id', '=', self.loan_id.id)])
            for rec in loan_operation_ids:
                if rec.loan_operation_type == 'increase_amount' and type != 'increase_amount' and rec.effective_date.month == effective_date.month and rec.effective_date.year == effective_date.year:
                    raise ValidationError(_('During this month %s has already request for increase amount so kindly check loan operation request!!' % employee.name))
        ...
        for res in self:
            if values.get('loan_operation_type') == 'loan_payment' or values.get('loan_payment_type') == 'partially' or values.get('payment_amount'):
                if values.get('payment_amount') and values.get('payment_amount') > res.loan_id.amount_to_pay:
                    raise UserError(_('Payment amount should be less then loan amount to pay!'))
                if values.get('payment_amount') and res.loan_payment_type == 'partially' and values.get('payment_amount') >= res.loan_id.amount_to_pay:
                    raise UserError(_('Payment amount should be less then loan amount to pay in partially payment!'))
        ...
        for loan in self:
            if loan.payment_type == 'by_account' or loan.loan_operation_type == 'increase_amount':
                ...
                debit_vals = {
                    ...
                    # 'loan_id': loan.id,
                    'analytic_tag_ids': [(6, 0, loan.analytic_tag_ids)],
                }
                credit_vals = {
                    'name': loan_name,
                    'partner_id': loan.employee_id.address_home_id.id,
            ...
            if loan.effective_date:
                loan_line_id = loan.loan_id.loan_lines.filtered(lambda l: not l.operation_id and l.date and l.date.month == loan.effective_date.month and l.state in ['draft', 'paid'])
                previous_ids = loan.loan_id.loan_lines.filtered(lambda l: not l.operation_id and l.date and l.date <= loan.effective_date and l.state in ['draft', 'paid'])
                remaining_ids = loan.loan_id.loan_lines.filtered(lambda l: not l.operation_id and l.date and l.date > loan.effective_date and l.state in ['draft', 'paid'])
            ...
            if loan.loan_operation_type == 'loan_payment':
                loan_line_id = loan.loan_id.loan_lines.filtered(lambda l: l.state in ['draft'])
                previous_ids = loan.loan_id.loan_lines.filtered(lambda l: l.state in ['draft'])
                remaining_ids = loan.loan_id.loan_lines.filtered(lambda l: l.state in ['draft'])
                if loan_line_id:
                    loan_line_id = loan_line_id[0]
                    loan_line_id.operation_id = loan.id
                if remaining_ids:
            ...
            if loan.loan_operation_type == 'loan_payment' and loan.loan_payment_type == 'fully':
                if remaining_ids:
                    for rl_id in remaining_ids:
                        rl_id.state = 'cancel'
                amount = loan.payment_amount
                amount = loan.loan_id.loan_amount - loan.loan_id.amount_paid
                slip_line_data = {'loan_id': loan.loan_id.id, 'operation_id': loan.id, 'employee_id': loan.employee_id.id,
                                  'date': fields.datetime.now(), 'amount': amount}
                installment_id = self.env['installment.line'].create(slip_line_data)
                loan.operation_applied = True
                loan.loan_id.state = 'done'

            if not loan_line_id:
                raise UserError(_('There is no any installment schedule are available or already another operation performed in the installment Schedule.'))
            if loan.loan_operation_type == 'skip_installment' and loan.effective_date:
                if loan_line_id:
                    last_id = sorted(loan.loan_id.loan_lines, key=lambda k: k.id, reverse=True)
                    if last_id:
                        last_id = last_id[0]
                        new_line_id = last_id.copy(default={'date': last_id.date + relativedelta(months=1)})
                        last_id.amount = loan_line_id.amount
                    loan_line_id.state = 'skip'
            elif loan.loan_operation_type == 'loan_payment' and previous_ids and remaining_ids: #and loan.effective_date
                amount = loan.loan_id.loan_amount
                ...
                if loan_line_id:
                    ...
                    if loan.loan_payment_type == 'partially' and remaining_ids:
                        ...
                        # remaining_diff = loan.loan_id.loan_amount - previous_amount - loan.payment_amount
                        ...
                        slip_line_data = {'loan_id': loan.loan_id.id, 'operation_id': loan.id, 'employee_id': loan.employee_id.id,
                              'date': fields.datetime.now(), 'amount': loan.payment_amount}
                        installment_id = self.env['installment.line'].create(slip_line_data)
                        loan.operation_applied = True
                    # elif loan.loan_payment_type == 'fully' and remaining_ids:
                    #     for rl_id in remaining_ids:
                    #         rl_id.state = 'cancel'
            elif loan.loan_operation_type == 'increase_amount' and loan.effective_date and loan.loan_amount:
                amount_to_pay = loan.loan_id.amount_to_pay + loan.loan_amount
                loan_amount = loan.loan_amount + loan.loan_id.loan_amount
                loan.loan_id.write({'loan_amount': loan_amount, 'amount_to_pay': amount_to_pay})
                remaining_ids = loan.loan_id.loan_lines.filtered(lambda l: l.date and l.date >= loan.effective_date and l.state in ['draft'])
                # remaining_ids = loan.loan_id.loan_lines.filtered(lambda l: l.date and l.date >= loan.effective_date and l.state in ['draft', 'paid'])
                if remaining_ids:
    ...
    #                                                                    ('loan_operation_type', 'in', ['increase_amount', 'loan_payment'])
    #                                                                    ])
    #     for rec in loan_operation_ids:
    ...
    #             #rec.loan_id.write({'loan_amount': loan_amount, 'amount_to_pay': amount_to_pay, 'deduction_amount': amount_to_pay / months})
    ...
    #             # rec.loan_id.with_context({'schedule_id': rec.loan_next_schedule_id}).make_calculation()
    ...
    #         if self._context.get('run_manually') and rec.loan_operation_type == 'loan_payment' and rec.payment_type == 'by_payslip':
