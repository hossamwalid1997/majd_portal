...
class HrLoan(models.Model):
    ...
    due_date = fields.Date('Loan Payment End Date', tracking=True, compute='_calculate_due_date', store=True)
    ...
    loan_lines = fields.One2many('hr.employee.loan.line', 'loan_id', string="Loan Line", index=True)
        ...
        for loan in self:
            ...
            for l_id in range(1, duration + 1):
                ...
                if loan.emi_based_on == 'amount' and duration == l_id:
                    vals.update({'amount': abs(((duration -1) * loan.deduction_amount) - loan.loan_amount)})
...
class HrEmployeeLoanLine(models.Model):
    ...
    state = fields.Selection([('draft', 'Pending'), ('skip', 'Skipped'), ('paid', 'Paid'), ('cancel', 'Cancel')],
