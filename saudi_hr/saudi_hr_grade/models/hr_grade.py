...
class HrGrade(models.Model):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        ...
        if context.get('job_id'):
            args.append(('id', 'in', self.env['hr.job'].browse(context.get('job_id')).grade_id.ids))
        return super(HrGrade, self).name_search(name, args=args, operator=operator, limit=limit)
