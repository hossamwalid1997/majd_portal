...
class Accommodation(models.Model):
    ...
    state = fields.Selection(hotel_states, string='Status', tracking=True, default='draft')
    ...
    reason_id = fields.Many2one('stay.purpose', required=True, string='Purpose of Stay', tracking=True)
    ...
    def accommodation_booked(self):
        ...
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_("Hello, %s <br/>Your Accommodation Request is booked.<br/>Check In Date: %s <br/>Check Out Date: %s <br/>Check Further Details In Accommodation Request.") % (self.employee_id.name,self.check_in_time,self.check_out_time))
        self.state = 'booked'
    ...
    def accommodation_refuse(self):
        ...
        self.ensure_one()
        self.state = 'refuse'
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Accommodation Refused.'))
    ...
    def set_draft(self):
        ...
        self.ensure_one()
        self.state = 'draft'
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Accommodation Created.'))
    ...
    def accommodation_over(self):
        ...
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Accommodation Stay Over.'))
    ...
    def accommodation_validate(self):
        ...
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Accommodation Approved.'))
...
class AccommodationLine(models.Model):
    ...
    accommodation_type = fields.Selection([('villa', 'Villa'), ('hotel', 'Hotel')], string='Stay In', required=True, tracking=True,
                                           default='villa')
    ...
    def create(self, values):
        ...
        if values.get('accommodation_id'):
            ...
            self.time_in_range(values['accommodation_id'], values.get('check_in_time'), values.get('check_out_time'))
    ...
    def write(self, values):
        ...
        for line in self:
            if values.get('accommodation_id'):
                ...
                self.time_in_range(line.accommodation_id.id, values.get('check_in_time') if values.get('check_in_time') else line.check_in_time, values.get('check_out_time') if values.get('check_out_time') else line.check_out_time)
    ...
    def time_in_range(self, accommodation_id=False, check_in_time=False, check_out_time=False):
        date_format = '%Y-%m-%d %H:%M:%S'
        check_in_time_formatted = check_in_time
        check_out_time_formatted = check_out_time
        if not isinstance(check_in_time_formatted, datetime):
            check_in_time_formatted = datetime.strptime(check_in_time, date_format)
        if not isinstance(check_out_time_formatted, datetime):
            check_out_time_formatted = datetime.strptime(check_out_time, date_format)
        accommodation_line = self.env['accommodation.line'].search([('accommodation_id', '=',accommodation_id)])
        for line in accommodation_line:
            if line.check_in_time <= check_in_time_formatted and line.check_out_time >= check_in_time_formatted:
                raise UserError(_("Time Conflict Please Check!"))
            elif line.check_in_time <= check_out_time_formatted and line.check_out_time >= check_out_time_formatted:
                raise UserError(_("Time Conflict Please Check!"))
        return {}
