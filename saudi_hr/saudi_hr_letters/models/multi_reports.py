...
class MultiReports(models.Model):
    ...
    def letter_confirm(self):
        ...
        self.state = 'confirm'
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Request Submitted.'))

    def letter_inprogress(self):
        self.ensure_one()
        self.state = 'inprogress'
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Letter request is in progress.'))
    ...
    def letter_approve(self):
        ...
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Letter request is approved.'))
    ...
    def letter_refuse(self):
        ...
        self.message_post(message_type="email", subtype_xmlid='mail.mt_comment', body=_('Letter request is refused.'))
    ...
    def unlink(self):
        for rec in self:
            if rec.state not in ['draft', 'refuse']:
                raise UserError(_('you can not delete record which is in %s stage.')%rec.state)
