...
class EmployeeAdvanceSalary(models.Model):
    ...
    def action_mail_send(self, position=None):
        ...
        try:
            if self.state == 'done':
                template_id = self.env.ref('sync_employee_advance_salary.email_template_advance_salary_request_done')
            elif self.state == 'refuse':
                template_id = self.env.ref('sync_employee_advance_salary.email_template_advance_salary_request_refuse')
        except ValueError:
            template_id = False
