...
class TransferEmployee(models.Model):
    ...
    analytic_tag_ids = fields.Many2many('account.analytic.tag', 'account_account_analytic_tag', string='Analytic Tags', readonly=True)
    ...
    new_analytic_tag_ids = fields.Many2many('account.analytic.tag', 'new_account_account_analytic_tag', string='Analytic Tags', tracking=True)
    ...
    def onchange_employee(self):
        ...
        if self.employee_id:
            ...
            if contract_ids:
                ...
                self.analytic_tag_ids = contract_ids[0].analytic_tag_ids and [(6, 0, contract_ids[0].analytic_tag_ids.ids)] or False
        ...
        if self.effective_date <= datetime.today().date():
            if self.hr_contract_id.date_end and self.effective_date >= self.hr_contract_id.date_end:
                vals = {'name': self.employee_id.name,
                        ...
                        'analytic_tag_ids': self.new_analytic_tag_ids and [(6, 0, self.new_analytic_tag_ids.ids)] or False,
