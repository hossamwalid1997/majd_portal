...
class HRApplicant(models.Model):
    ...
    def onchange_department_id(self):
        ...
        if self.department_id:
            job_ids = requisition_obj.search([('department_id', '=', self.department_id.id)]).mapped('job_id')
            if job_ids:
    ...
    def case_close_with_emp(self):
        ...
        employee_action = self.env["ir.actions.actions"]._for_xml_id('hr.open_view_employee_list')
        if emp_id:
    ...
    def send_offer_letter(self):
        ...
        try:
            template_id = ir_model_data._xmlid_lookup('saudi_hr_recruitment_custom.email_template_applicant_offer')[2]
        except ValueError:
            template_id = False
        try:
