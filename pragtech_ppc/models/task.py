...
class Task(models.Model):
    ...
    is_task = fields.Boolean('Task',copy=True)
    ...
    parent_task_id = fields.Many2one('project.task', 'Parent Group', store=True,copy=False)
    parent_group_id = fields.Many2one('project.task', 'Parent Group',copy=False)
    parent_id = fields.Many2one('project.task', string="Parent", help="Set field for task hirarchy.",copy=False)
    ...
    is_wbs = fields.Boolean('WBS', domain=[('is_task', '=', False), ('is_task', '=', True)], copy=True)
    ...
    def get_unit_price(self):
        """Method to calculate the unit_price of wbs."""
        for task in self:
            
           
            task.update({
                'unit_price': task.total_wbs_value,
                'uom_id': 1,
                'quantity': 1,
            })
    ...
    def action_product_done(self):
        ...
        products_wbs_ = self.env['product.product'].create(
                        {"name": self.name, "lst_price": self.unit_price, "wbs_id": self.id,
                         "detailed_type": "consu", })
    ...
    def create_task_wbs(self, group_obj, parent_id=None, parent=None, parent_task=None, parent_group=None):
        ...
        for material in obj.task_material_line:
            material_data = {
                ...
                'material_uom': material.material_id.uom_id.id,
                ...
                'wbs_id': wbs,
                'sub_total' : material.material_uom_qty * material.material_rate,
        ...
        for labour in obj.task_labour_line:
            labour_data = {
                ...
                'task_category': obj.category_id.id,
                'wbs_id': wbs,
                'sub_total':labour.labour_uom_qty * labour.labour_rate,
            }
