...
class TaskLabourLine(models.Model):
    ...
    labour_profit_id = fields.Float('Profit')
   
    labour_overhead = fields.Float('Overhead ')

    labour_line_id = fields.Many2one('project.task', string='Project Task',copy=False)
    ...
    def _onchange_labour_id(self):
        if self.labour_id:
            self.update({
                ...
                'sub_total': self.labour_id.rate * self.labour_uom_qty
            })
