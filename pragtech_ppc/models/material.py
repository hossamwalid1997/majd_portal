...
class TaskMaterialLine(models.Model):
    ...
    material_line_id = fields.Many2one('project.task', string='Project Task',copy=False)
    wbs_id = fields.Many2one('project.task', string='WBS',copy=False)
    group_id = fields.Many2one('project.task', related='material_line_id.parent_task_id',  string='Group')
    material_category = fields.Many2one('product.category', related='material_id.categ_id',  string='Material Category')
    task_category = fields.Many2one('task.category', related='material_line_id.category_id',  string='Task Category')
    ...
    balanced_requisition=fields.Float('Balanced Requisition',compute='get_balance_requisition')
    waste = fields.Float('Waste %')
    transfer = fields.Float('Transfer')
    profit_id = fields.Float('Profit')
    loading = fields.Float('Loading %' )
    overhead = fields.Float('Overhead %')
    
    last_purchase = fields.Float('Last Purchase' )
    last_quotation = fields.Float('Last Quotation')
...
#             })
    ...
    def _onchange_material_id(self):
        if self.material_id:
            purchase_ids = self.env['purchase.order.line'].search([("product_id","=",self.material_id.id)], limit=1)
            for purchase in purchase_ids:
                
                self.update({
                'last_purchase': purchase.price_unit,
            })
            sale_ids = self.env['sale.order.line'].search([("product_id","=",self.material_id.id)], limit=1)
            for sale in sale_ids:
                 self.update({
                'last_quotation': sale.price_unit,
            })
            self.update({
...
class MaterialLibrary(models.Model):
    ...
    uom_ids = fields.One2many('uom.uom', string='Unit of Measure category', related='material_id.uom_id.category_id.uom_ids')
    material_uom = fields.Many2one('uom.uom', string='Unit of Measure', required=True, domain="[('id','in',uom_ids)]")
    material_uom_qty = fields.Float(string='Quantity', default=1.0)
    material_rate = fields.Float(string='Rate', default=1.0)
    ...
    def _onchange_material_id(self):
        if self.material_id:
            ...
            # res = {'domain': {'material_uom': "[('id', 'not in', False)]"}}
            # uom_list = []
            # for uom in self.material_id.uom_id.category_id.uom_ids:
            #     uom_list.append(uom.id)
            # res['domain']['material_uom'] = "[('id', 'in', %s)]" % uom_list
            # return res
