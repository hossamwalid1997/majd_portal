...
class ApprovalWizard(models.TransientModel):
    ...
    def update_status(self):
        ...
        active_ids = context.get('active_ids')
        model_id = self.env['ir.model'].search([('model', '=', active_model)])
        stage_obj = self.env['stage.transaction'].search([('from_stage', '=', self.current_stage.id),('to_stage', '=', self.new_stage.id), ('model', '=', model_id.id)])
        if self.env.user.id not in stage_obj.groups.users.ids:
            raise UserError(_("The user does not have the right to change stages"))
