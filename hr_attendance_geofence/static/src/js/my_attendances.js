odoo.define('hr_attendance_geofence.my_attendances', function (require) {
    ...
    var MyAttendances= MyAttendances.include({
        ...
        _toggle_gmap: function(){
            var self = this;
            if (self.$(".gmap_kisok_toggle").hasClass('fa-angle-double-up')) {
                self.$('.gmap_kisok_view').toggle('hide');
                self.$("i", self).toggleClass("fa-angle-double-up fa-angle-double-down");
                self._initMap();
            }else{
                self.$('.gmap_kisok_view').toggle('show');
                self.$("i", self).toggleClass("fa-angle-double-down fa-angle-double-up");
                self._initMap();
            }            
        },
        _initMap: function(){
