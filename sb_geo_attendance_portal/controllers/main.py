...
class HrAttendanceController(http.Controller):
    ...
    # @http.route('/portal_attendance_geo/_get_gmap_api_key', type='json', auth='public', website=True)
    # def _get_gmap_api_key(self):
    #     key = request.env['ir.config_parameter'].sudo().get_param('portal_attendance_geo.gmap_api_key')
    #     return json.dumps({
    #         'gmap_api_key': key or ''
    #     })

    @http.route('/sb_geo_attendance_portal/search_read/get_employee_data', type='json', auth='user', website=True)
...
class HrAttendancePortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'hr_attendance_count' in counters:
            hr_attendance_count = HrAttendance.sudo().search_count(
                [(
                    'employee_id',
                    '=',
                    employee and employee.id or False
                )
                ])
    ...
    def portal_my_hr_attendances(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None,
                                 search_in='all', **kw):
        ...
        searchbar_filters = {
            ...
            'week': {'label': _('This week'), 'domain': [('check_in', '>=', date_utils.start_of(today, "week")),
                                                         ('check_out', '<=', date_utils.end_of(today, 'week'))]},
            'month': {'label': _('This month'), 'domain': [('check_in', '>=', date_utils.start_of(today, 'month')),
                                                           ('check_out', '<=', date_utils.end_of(today, 'month'))]},
            'year': {'label': _('This year'), 'domain': [('check_in', '>=', date_utils.start_of(today, 'year')),
                                                         ('check_out', '<=', date_utils.end_of(today, 'year'))]},
            'quarter': {'label': _('This Quarter'),
                        'domain': [('check_in', '>=', quarter_start), ('check_out', '<=', quarter_end)]},
            'last_week': {'label': _('Last week'),
                          'domain': [('check_in', '>=', date_utils.start_of(last_week, "week")),
                                     ('check_out', '<=', date_utils.end_of(last_week, 'week'))]},
            'last_month': {'label': _('Last month'),
                           'domain': [('check_in', '>=', date_utils.start_of(last_month, 'month')),
                                      ('check_out', '<=', date_utils.end_of(last_month, 'month'))]},
            'last_year': {'label': _('Last year'),
                          'domain': [('check_in', '>=', date_utils.start_of(last_year, 'year')),
                                     ('check_out', '<=', date_utils.end_of(last_year, 'year'))]},
        }
        ...
        if search and search_in:
            ...
            if search_in in ('all', 'all'):
                search_domain = OR(
                    [search_domain, ['|', ('check_in', 'ilike', search), ('check_out', 'ilike', search)]])
        ...
        hr_attendances = HrAttendance.search(domain, order=sort_order, limit=self._items_per_page,
                                             offset=pager['offset'])
        ...
        return request.render("sb_geo_attendance_portal.portal_my_hr_attendances", values)

    #
    # @http.route(['/my/hr_attendance/<int:attendance_id>'], type='http', auth="public", website=True)
    # def portal_my_pos_order(self, attendance_id, report_type=None, access_token=None, message=False, download=False,
    #                         **kw):
    #     try:
    ...
    #
    #     return request.render('portal_attendance_geo.portal_my_hr_attendance', values)
    #
    ...
    def create_new_attendance(self, access_token=None):
        ...
        return request.render("sb_geo_attendance_portal.my_attendance_create_new", values)
